/**
 * Test Validation Helpers for Innovator Agent Testing
 *
 ...
 * - Innovator-specific behaviors: Intro opportunities, prospect matching, credit tracking
 */

import type { AgentResponse, AgentActionType } from '@yachtparty/shared';

/**
 * Call 1 Decision Output structure (similar to Concierge but adapted for Innovator)
 */
...
/**
 * Re-engagement Decision Output structure
 */
export interface ReengagementDecisionOutput {
...
/**
 * Verify Call 1 Decision Output (User Message or System Event)
 */
...
export function verifyReengagementDecision(
  ...
  expected: {
    ...
    scenario?: string; // Expected scenario
...
export function verifyCall2Messages(
  ...
  expected: {
    ...
    mentionsCredits?: boolean; // Should mention credits/bounty
    mentionsIntro?: boolean; // Should mention introduction
  }
) {
  const errors: string[] = [];
  ...
  // Verify mentions credits
  if (expected.mentionsCredits) {
    const allMessages = response.messages.join(' ');
    if (!/(credit|bounty)/i.test(allMessages)) {
      errors.push('Expected messages to mention credits or bounty');
    }
  }

  // Verify mentions intro
  if (expected.mentionsIntro) {
    const allMessages = response.messages.join(' ');
    if (!/(intro|introduction|connect)/i.test(allMessages)) {
      errors.push('Expected messages to mention introduction or connection');
    }
  }
...
/**
 * Helper to check if tone is helpful but not overeager (inherited from Concierge)
 */
...
/**
 * Helper to check if tone is professional and solution-focused (appropriate for innovator)
 */
export function checkToneProfessional(message: string): boolean {
...
/**
 * Helper to check if tone is brief (under 3-4 sentences typically)
 */
...
/**
 * Verify intro opportunity was created correctly
 */
export function verifyIntroOpportunityCreated(
  ...
  // Note: Changed from create_intro_opportunity to offer_introduction
  // User offering to introduce someone is now offer_introduction
  const createIntroAction = actions.find(a => a.type === 'offer_introduction');

  if (!createIntroAction) {
    throw new Error('No offer_introduction action found');
  }

  const errors: string[] = [];

  if (expected.prospectName && createIntroAction.payload.prospect_name !== expected.prospectName) {
    errors.push(`Expected prospect_name '${expected.prospectName}', got '${createIntroAction.payload.prospect_name}'`);
  }

  if (expected.connectorUserId && createIntroAction.payload.connector_user_id !== expected.connectorUserId) {
    errors.push(`Expected connector_user_id '${expected.connectorUserId}', got '${createIntroAction.payload.connector_user_id}'`);
  }

  if (expected.bountyCredits && createIntroAction.payload.bounty_credits !== expected.bountyCredits) {
    errors.push(`Expected bounty_credits ${expected.bountyCredits}, got ${createIntroAction.payload.bounty_credits}`);
  }

  if (errors.length > 0) {
    throw new Error(`Intro Opportunity Verification Failed:\n${errors.join('\n')}`);
  }
...
/**
 * Verify credit balance was updated correctly
 */
export function verifyCreditUpdate(
  actions: AgentResponse['actions'],
  expected: {
    userId: string;
    amount: number;
  ...
  const creditAction = actions.find(a => a.type === 'update_credits');

  if (!creditAction) {
    throw new Error('No update_credits action found');
  }

  const errors: string[] = [];

  if (creditAction.payload.user_id !== expected.userId) {
    errors.push(`Expected user_id '${expected.userId}', got '${creditAction.payload.user_id}'`);
  }

  if (creditAction.payload.amount !== expected.amount) {
    errors.push(`Expected amount ${expected.amount}, got ${creditAction.payload.amount}`);
  }

  if (creditAction.payload.operation !== expected.operation) {
    errors.push(`Expected operation '${expected.operation}', got '${creditAction.payload.operation}'`);
  }
